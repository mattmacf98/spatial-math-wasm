...
export function getPosesFromTrajectory(frameSystem: string, trajectory: string, frameName: string): unknown[] | undefined {
    if (!isWasmReady()) {
        console.warn('WASM is not ready');
        return undefined;
    }
    
    if (typeof globalThis.getPosesFromTrajectory !== 'function') {
        console.error('getPosesFromTrajectory function not available in WASM module');
        return undefined;
    }
    return globalThis.getPosesFromTrajectory(frameSystem, trajectory, frameName);
}
